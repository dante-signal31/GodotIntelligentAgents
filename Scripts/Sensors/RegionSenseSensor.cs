...
/// <summary>
/// Represents a region sense sensor that detects and processes signals
...
/// </summary>
[Tool]
public abstract partial class RegionSenseSensor<T>: 
    Node2D, IRegionSenseSensor, ISensor, ISignalSensor where T: RegionSenseModality
{
    ...
    /// <summary>
    /// Scene path to the RegionSenseManager node this sensor will subscribe to.
    /// </summary>
    ...
    public HashSet<Node2D> DetectedObjects
    {
        get
        {
            ...
            // Copy the array to avoid concurrent modification exceptions.
    ...
    public PriorityQueue<DetectedSignal, float> DetectedSignals {
        get
        {
            PriorityQueue<DetectedSignal, float> detectedSignalsQueue = new();
            // Copy the array to avoid concurrent modification exceptions.
            DetectedSignal[] detectedSignals = _detectionBuffer.ToArray();
            
            foreach (var detectedSignal in detectedSignals)
            {
                detectedSignalsQueue.Enqueue(detectedSignal, detectedSignal.Signal.Strength);
            }
            
            return detectedSignalsQueue;
        }
    }
